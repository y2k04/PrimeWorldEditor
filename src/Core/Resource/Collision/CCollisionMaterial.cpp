...
CColor CCollisionMaterial::SurfaceColor(EGame Game) const
{
    ...
    static const std::unordered_map<ECollisionFlag, CColor> skTypeToColor = {
    ...
    const auto FindColor = skTypeToColor.find(SurfType);
    return (FindColor == skTypeToColor.end() ? CColor::White() : FindColor->second);
}
